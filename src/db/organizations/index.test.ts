...
describe("Organizations manual SQL: findManyByFilter", () => {
  ...
  async function findAllByFilter(
    filterModel: Omit<OrganizationFilterModel, "pageKey">
  ) {
    const retrievedVenues: Awaited<
      ReturnType<
        ReturnType<typeof getDbClient>["organizationEntity"]["findManyByFilter"]
      >
    > = [];
    for (
      ...
      const result = await getDbClient().organizationEntity.findManyByFilter({
        ...filterModel,
        pageKey,
      });
  ...
  > {
    return Promise.all(
      result.map(async (it) => ({
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        ...(await OrganizationDao.getReferenceById(it.profileId))!,
  ...
  test("sorts by and prioritizes name similarity when a name query is given", async () => {
    ...
    expect(result[0]).toMatchObject<{ name: TestOrganizationNameType }>({
      name: "Storbandet",
    });
    expect(result[1]).toMatchObject<{ name: TestOrganizationNameType }>({
      name: "Stora Bandet",
    });
  });
  ...
  test("sorts by name no name query is given", async () => {
    ...
    expect(result).toHaveLength(Object.keys(organizations).length);
    result.forEach((it) => {
  ...
  test("can limit results to organizations with a specific member", async () => {
    const result = await findAllByFilter({
      nameQuery: null,
      tags: new Set(),
      memberIds: new Set([individuals["Stina Lundgren"].id]),
    }).then(expandToReferenceModel);
    ...
    expect(
      ...
      individuals["Stina Lundgren"].organizations
        .map((it) => it.profileReference.name)
        .sort((a, b) => a.localeCompare(b, "sv-SE"))
    );
  });

  test("can limit results to organizations with specific members", async () => {
    const result = await findAllByFilter({
      nameQuery: null,
      tags: new Set(),
      memberIds: new Set([
        individuals["Stina Lundgren"].id,
        individuals["Joseph Axelsson"].id,
      ]),
    }).then(expandToReferenceModel);
    ...
    expect(
      ...
      individuals["Stina Lundgren"].organizations
        .filter((org) =>
          individuals["Joseph Axelsson"].organizations
            .map((it) => it.profileReference.id)
            .includes(org.profileReference.id)
        )
        .map((it) => it.profileReference.name)
        .sort((a, b) => a.localeCompare(b, "sv-SE"))
    );
  ...
  test("can limit results to a single tag", async () => {
    // Arrange
    const allTags = Object.values(OrganizationTag);
    let tagFilter = new Set<OrganizationTag>();
  ...
  test("invalid page key returns empty result set", async () => {
    const filter: OrganizationFilterModel = {
      nameQuery: null,
      tags: new Set(),
      memberIds: new Set(),
      pageKey: null,
    ...
    const firstPage =
      await getDbClient().organizationEntity.findManyByFilter(filter);

    // Sanity check: using a valid id should produce results
    filter.pageKey = firstPage[0].profileId;
    await expect(
      getDbClient().organizationEntity.findManyByFilter(filter)
